...
struct AdvancedTransitionsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 40) {
                Text("Twitter Heart Animation")
                    .font(.headline)
                Text("하트를 탭하면 confetti transition 적용")
                    .font(.caption)
                ...
                Text("Gradient Confetti")
                    .font(.headline)

                GradientConfettiDemo()
            }
            .padding()
        }
        .navigationTitle("Advanced Transitions")
    }
}

struct GradientConfettiDemo: View {
    @State private var showStar = false

    var body: some View {
        VStack(spacing: 20) {
            Text("그라디언트 ShapeStyle 적용")
                .font(.caption)
                .foregroundStyle(.secondary)
